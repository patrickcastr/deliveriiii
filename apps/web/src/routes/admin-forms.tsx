...
export default function AdminForms() {
  ...
  async function load() {
    const res = await fetch('/api/v1/forms/templates', { credentials: 'include' });
    if (res.ok) {
  ...
  const onSave = handleSubmit(async (data) => {
    ...
    try {
      ...
      const csrf = getCsrf();
      if (selected) {
        const res = await fetch(`/api/v1/forms/templates/${selected.id}`, {
        ...
        const res = await fetch('/api/v1/forms/templates', {
  ...
  async function publish(t: Template) {
    setBusy(true); setError(null);
    const csrf = getCsrf();
    const res = await fetch(`/api/v1/forms/templates/${t.id}/publish`, { method: 'POST', headers: { 'X-CSRF-Token': csrf }, credentials: 'include' });
  ...
  async function archive(t: Template) {
    setBusy(true); setError(null);
    const csrf = getCsrf();
    const res = await fetch(`/api/v1/forms/templates/${t.id}`, { method: 'DELETE', headers: { 'X-CSRF-Token': csrf }, credentials: 'include' });
