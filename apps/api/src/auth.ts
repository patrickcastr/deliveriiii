...
export function signAccess(user: JwtUser) {
  ...
  return jwt.sign(payload, config.jwt.accessSecret as string, { expiresIn: config.jwt.accessTtl } as SignOptions);
}
export function signRefresh(user: JwtUser) {
  const payload: RefreshTokenPayload = { ...user, type: 'refresh', jti: randomUUID() };
  return { token: jwt.sign(payload, config.jwt.refreshSecret as string, { expiresIn: config.jwt.refreshTtl } as SignOptions), jti: payload.jti };
}

export function verifyAccess(token: string): AccessTokenPayload {
  return jwt.verify(token, config.jwt.accessSecret as string) as AccessTokenPayload;
}
export function verifyRefresh(token: string): RefreshTokenPayload {
  return jwt.verify(token, config.jwt.refreshSecret as string) as RefreshTokenPayload;
}
